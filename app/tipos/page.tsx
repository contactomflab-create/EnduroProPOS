...
export default function Page() {
  ...
  const fetch = useCallback(async () => {
    const { data } = await supabase.from('tipos_producto').select('*').order('nombre')
  ...
  const handleSave = async () => {
    ...
    if (selected) {
      const { error } = await supabase.from('tipos_producto').update({ nombre: form.nombre, descripcion: form.descripcion || null }).eq('id', selected.id)
    ...
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      ...
      <main style={{ flex: 1, padding: '32px', overflow: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28, flexWrap: 'wrap', gap: 16 }}>
          <div>
            <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 32, fontWeight: 800 }}>TIPOS DE PRODUCTO</div>
          ...
          <div className="modal-overlay" onClick={() => setShowModal(false)}>
            <div className="modal" style={{ maxWidth: 400 }} onClick={e => e.stopPropagation()}>
              <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 22, fontWeight: 700, marginBottom: 24 }}>
                {selected ? 'EDITAR' : 'NUEVO'} TIPOS DE PRODUCTO
              </div>
