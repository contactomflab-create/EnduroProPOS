...
export default function Page() {
  ...
  const handleSave = async () => {
    ...
    if (selected) {
      const { error } = await supabase.from('marcas').update({ nombre: form.nombre, descripcion: form.descripcion || null }).eq('id', selected.id)
      if (error) return showToast('Error al actualizar', 'error')
      showToast('Actualizado correctamente', 'success')
    } else {
      const { error } = await supabase.from('marcas').insert({ nombre: form.nombre, descripcion: form.descripcion || null })
    ...
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      ...
      <main style={{ flex: 1, padding: '32px', overflow: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28, flexWrap: 'wrap', gap: 16 }}>
          <div>
            <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 32, fontWeight: 800 }}>MARCAS</div>
