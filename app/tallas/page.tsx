...
export default function Page() {
  ...
  const fetch = useCallback(async () => {
    const { data } = await supabase.from('tallas').select('*').order('nombre')
  ...
  const handleSave = async () => {
    ...
    if (selected) {
      const { error } = await supabase.from('tallas').update({ nombre: form.nombre, descripcion: form.descripcion || null }).eq('id', selected.id)
      if (error) return showToast('Error al actualizar', 'error')
      showToast('Actualizado correctamente', 'success')
    } else {
      const { error } = await supabase.from('tallas').insert({ nombre: form.nombre, descripcion: form.descripcion || null })
      if (error) return showToast('Ya existe ese nombre', 'error')
      showToast('Talla creada', 'success')
    }
    setShowModal(false)
    fetch()
  }
    ...
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      ...
      <main style={{ flex: 1, padding: '32px', overflow: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28, flexWrap: 'wrap', gap: 16 }}>
          <div>
            <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 32, fontWeight: 800 }}>TALLAS</div>
